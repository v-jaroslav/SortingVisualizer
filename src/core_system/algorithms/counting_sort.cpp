...
namespace SortingVisualizer::CoreSystem::Algorithms
{
    ...
    {
        // For counting sort, we need a histogram, that will keep track how many times did value appear, which will be stored at histogram[value].
        ...
        // Next, we need "i" which will point to the next value in array which will record into our histogram.
    ...
    void CountingSort::Step() 
    {
        ...
        {
            // As long as we haven't recorded allt he values of the array in the histogram.
            // Then grabh this->array[i], and grab its value, and use that value as index of histogram, and increment number at that index.
            // That way we indicate that this element "has appeared one more time".
            ++this->histogram[this->array[this->i].GetValue()];
            
            // And also set the focus on the i-th element. And move on to the next element to record into the histogram.
            this->array[this->i].SetFocus();
            ++this->i;
        }
        else if (this->free < this->array.GetNumberOfVisibleElements()) 
        {
            // If we are done with recording the elements in histogram, time to alter the array.
            // As long as there are free slots in the array, then we aren't done. In that case let us find the first value "k".
            // That does not appear 0 times, which means we can add it to the array.
